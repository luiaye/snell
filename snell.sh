...
# 作者: luiaye
# 日期: 2026年8月
# 描述: 安装/更新 snell-server、主用户初始化、子用户多开、查看配置、删除用户、重启、卸载
# =========================================
...
latest_version_v6() {
    ...
    version=$(curl -fsSL https://kb.nssurge.com/surge-knowledge-base/zh/release-notes/snell 2>/dev/null \
    | grep -oE 'snell-server-v6\.[0-9]+\.[0-9]+b[0-9]+' \
    | head -n1 \
    | sed 's/snell-server-//')
...
install_or_update_binary() {
  ...
  if [[ "${SNELL_VERSION:-v6}" == "v6" ]]; then
    ver="$(latest_version_v6)"
  else
...
del_user() {
  ...
  for c in "${USERS_DIR}"/snell-*.conf; do
    ...
    if [[ -n "$ip" ]]; then
      echo "U${p} = snell, $(surge_host "$ip"), ${p}, psk = ${psk}, version = ${SNELL_VERSION_NUM:-6}, reuse = true, tfo = true    # 状态:${st}"
    else
...
detect_snell_major() {
  ...
  echo "$v" | grep -q 'v6' && echo "6" || echo "$v" | grep -q 'v5' && echo "5" || echo "unknown"
}
...
show_surge_lines() {
  ...
  if [[ -f "$MAIN_CONF" ]]; then
    ...
    p="$(grep -E '^listen' "$MAIN_CONF" | sed -n 's/.*::0:\([0-9]*\)/\1/p')"
    psk="$(grep -E '^psk' "$MAIN_CONF" | awk -F'=' '{print $2}' | tr -d ' ')"
    [[ -n "$ip4" ]] && echo "MAIN4 = snell, ${host4}, ${p}, psk = ${psk}, version = ${SNELL_VERSION_NUM:-6}, reuse = true, tfo = true"
    [[ -n "$ip6" ]] && echo "MAIN6 = snell, ${host6}, ${p}, psk = ${psk}, version = ${SNELL_VERSION_NUM:-6}, reuse = true, tfo = true"
  fi
  ...
  for conf in "${USERS_DIR}"/snell-*.conf; do
    ...
    p="$(grep -E '^listen' "$conf" | sed -n 's/.*::0:\([0-9]*\)/\1/p')"
    psk="$(grep -E '^psk' "$conf" | awk -F'=' '{print $2}' | tr -d ' ')"
    [[ -n "$ip4" ]] && echo "U${p}4 = snell, ${host4}, ${p}, psk = ${psk}, version = ${SNELL_VERSION_NUM:-6}, reuse = true, tfo = true"
    [[ -n "$ip6" ]] && echo "U${p}6 = snell, ${host6}, ${p}, psk = ${psk}, version = ${SNELL_VERSION_NUM:-6}, reuse = true, tfo = true"
  done
...
show_shadowtls_lines() {
  ...
  for svc in /etc/systemd/system/shadowtls-snell-*.service; do
    ...
    [[ -n "$ip4" && -n "$psk" ]] && echo "S${snell_port}4 = snell, ${host4}, ${stls_port}, psk = ${psk}, version = ${SNELL_VERSION_NUM:-6}, reuse = true, tfo = true, shadow-tls-password = ${stls_pwd}, shadow-tls-sni = ${stls_sni}, shadow-tls-version = 3"
    [[ -n "$ip6" && -n "$psk" ]] && echo "S${snell_port}6 = snell, ${host6}, ${stls_port}, psk = ${psk}, version = ${SNELL_VERSION_NUM:-6}, reuse = true, tfo = true, shadow-tls-password = ${stls_pwd}, shadow-tls-sni = ${stls_sni}, shadow-tls-version = 3"
  done
...
list_shadowtls() {
  ...
  for svc in /etc/systemd/system/shadowtls-snell-*.service; do
    ...
    if [[ -n "$psk" && ( -n "$ip4" || -n "$ip6" ) ]]; then
      [[ -n "$ip4" ]] && echo "${tag}4 = snell, ${host4}, ${stls_port}, psk = ${psk}, version = ${SNELL_VERSION_NUM:-6}, reuse = true, tfo = true, shadow-tls-password = ${stls_pwd}, shadow-tls-sni = ${stls_sni}, shadow-tls-version = 3"
      [[ -n "$ip6" ]] && echo "${tag}6 = snell, ${host6}, ${stls_port}, psk = ${psk}, version = ${SNELL_VERSION_NUM:-6}, reuse = true, tfo = true, shadow-tls-password = ${stls_pwd}, shadow-tls-sni = ${stls_sni}, shadow-tls-version = 3"
    else
...
menu() {
  clear
  echo -e "${CYAN}========================================${RESET}"
  echo -e "${CYAN}   Snell v6 + ShadowTLS 综合管理脚本${RESET}"
  echo -e "${CYAN}========================================${RESET}"
  echo "1) 安装/更新"
  echo "2) 新增子用户"
  echo "3) 删除子用户"
  ...
  while true; do
    ...
    case "${opt}" in
      1)
        ...
        read -rp "请选择 [1-2]: " sv
        if [[ "$sv" == "1" ]]; then
          export SNELL_VERSION="v5"
          export SNELL_VERSION_NUM="5"
        else
          export SNELL_VERSION="v6"
          export SNELL_VERSION_NUM="6"
        fi
        install_and_init_main || true
        ;;
      2) add_user || true ;;
      3) del_user || true ;;
